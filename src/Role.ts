...
export default class Role implements roleInterface{
    ...
    run(){
        const newRole = this;
        const { mapData, width, height, velocity, positionY, findPresentMapPointRun, mapPosition, screenWidth, positionX, getMapX } = newRole;
        let newX = positionX;
        let newY = positionY;
        ...
        const posX = getMapX(mapMoveLimitRage, mapData, mapPosition, positionX, screenWidth, velocity, width); // role在mapData地图(注意不是screenMap)的x坐标，因为有地图移动
        const pos = [posX, newY];
        const V = [...velocity];

        const point = findPresentMapPointRun(pos, newRole); // 当前role在mapData上的垂直映射点
        ...
        // 尾部渲染地图中点
        const lastMiddle = mapData[mapData.length - 1][0] - mapMoveLimitRage;

        //判断Y坐标是否约等于（ 当前x坐标与mapData上的映射点x坐标位置比对）。
...
//--------------------------------------------------------------水平左右移动逻辑
        ...
        if(!balance){ //
            ...
            if(newX <= 0 && newRole.velocity[0] < 0){ // 左侧撞墙
                newX = 0;
            } else if(newX + newRole.mapPosition[0] - newRole.screenWidth/2 >= newRole.mapData[newRole.mapData.length - 1][0] - width && velocity[0] >= 0){
                // 右侧撞墙
                newX = newRole.screenWidth - width;
            } else if ( newX >= mapMoveLimitRage &&  (newX + newRole.mapPosition[0] - newRole.screenWidth/2 <= lastMiddle)){
                // role相对于mapData的当前坐标：如果小于半屏则为newX，大于则为newRole.mapPosition[0]
                newX = mapMoveLimitRage;
                newRole.mapVelocity = velocity[0];
                // newRole.velocity[0] = 0;

                if(newX === mapMoveLimitRage && newRole.mapPosition[0] === mapMoveLimitRage && velocity[0] > 0){
                    V[0] = velocity[0] > 0 ? 0 : velocity[0];
                }
            }
        }
        newX = newX + V[0];
...
//--------------------------------------------------------------
    ...
    findPresentMapPointRun(pos: number[], newRole: any){
        const { mapData, width, height, velocity, tempHorizon } = newRole;
        let point: [number, number, number] = [pos[0], 0, 0]; // [number, number, number(0平台/1凸起/-1凹陷)]
        let roleBottom = pos[1]; // 角色底部,由于mapData中是以horizon为基准计算的
        let jumpStartHorizon = tempHorizon; // 起跳点记录
        for(let i = 0; i < mapData.length - 2; i++){
            let pointA: [number, number] = mapData[i],
                pointB: [number, number] = mapData[i + 1];
            if(pos[0] + width === pointA[0]){ // role右侧点与mapData点重合:台阶左下角底边点/跳跃（平抛）时台子左上角点/台子右上角点（错误处理台子右下角点）
            ...
            } else if(pos[0] === pointA[0]){  // role左侧点与mapData点重合，mapData点呈现高度下降趋势，跳上右侧台/从高台右上角跳下  动作
                if(pointA[1] < pointB[1]){
            ...
            if(pos[0] > pointA[0] && pos[0] + width < pointB[0]){ // role整体在平台上
                return  [pos[0], pointA[1], 0];
            } else if(pos[0] < pointA[0] && pos[0] + width > pointA[0] && pointA[0] === pointB[0]){ // mapData上必有上与下两个x相同的点在中间
    ...
    jump(){
        const newRole = this;
        const { findPresentMapPoint, width, height, velocity, positionY, getMapX, mapData, preFm, mapPosition, positionX, screenWidth, tempHorizon, horizon } = this;
        ...
        // 当前坐标与mapData之间的关系
        const mapMoveLimitRage = newRole.screenWidth/2; // 移动范围参数化，暂时不计入右侧范围
        const posX = getMapX(mapMoveLimitRage, mapData, mapPosition, positionX, screenWidth, velocity, width); // role在map地图(注意不是screenMap)的x坐标，因为有地图移动
        const pos = [posX, positionY]; // 当前role在mapData的坐标点，
        const point = findPresentMapPoint(pos, newRole); // 当前role在mapData上的垂直映射点


        // role当前在地图的Y坐标，mapData映射点
...
//---------------------Y向坐标处理
        // 当前x坐标与mapData的x坐标对比
        // 未作部分：如果大于mapMoveLimitRage(screenWidth/2)则v为0，此时转化为纵向jump，然后x向mapData继续移动，目前状态jump时mapData不动
        if(balance){ // 该结果在结束时可能不会出现，需要近似运算
            ...
            if(newRole.preFm === 1){                          // 动画开始
                ...
                newRole.mapVelocity = 0; // 清理mapData的v速度，不正确？

                newRole.button.space = keyStatus.off;// 关闭跳跃loading状态
                newRole.tempHorizon = mappingPointY; // 动画开始 动画结束，重新记录位置

            }
        } else if(rolePositionBottomY > mappingPointY){ // 跳起，未着地，未撞墙，由于墙由上下两个点绘制，level为role左侧，垂直映射到mapData上的点
        ...
        } else if(rolePositionBottomY < mappingPointY){// 撞墙，由于墙由上下两个点绘制，level为role左侧，垂直映射到mapData上的点
            V[0] = 0;
            newRole.preFm = newRole.preFm + 1;
...
//--------------------- mapData动作渲染role操作
        ...
        // 尾部渲染地图中点，正向。
        const rightEdge = mapData[mapData.length - 1][0] - mapMoveLimitRage;
        ...
        if(newX <= 0 && newRole.velocity[0] < 0){
            newX = 0;
        } else if(newX + newRole.mapPosition[0] - newRole.screenWidth/2 >= newRole.mapData[newRole.mapData.length - 1][0] - width){
            // 右侧撞墙
            newX = newRole.screenWidth - width;
        } else if ( pos[0] >= mapMoveLimitRage &&  (newX + newRole.mapPosition[0] - mapMoveLimitRage <= rightEdge)){     // mapData 移动区间
            // role相对于mapData的当前坐标
            newX = mapMoveLimitRage;
            newRole.velocity[0] = 0;
        } else {
...
// 定位，绘制
    ...
    // 查找当前坐标在mapData上的投影点，这个点以左侧作为基准
    // 精彩判断部分：
    // 判断x与x+width在mapData上的映射，如果两个映射坐标Y值不同（如果有x等值的点），则中间必定有点，如果中间没有点则role在台阶上，如果中间有点则role的一部分在台阶上，取y值高的点。
    findPresentMapPoint(pos: number[], newRole: any){
        const { mapData, width, height, velocity } = newRole;
        ...
        for(let i = 0; i < mapData.length - 2; i++){
            ...
            if(pos[0] + width === pointA[0]){ // role右侧点与mapData点重合，mapData点呈现高度上升趋势
                if(pointA[1] > pointB[1]){
                    if(roleBottom === pointA[1]){ // 与顶边重合
                        return [pos[0], pointA[1], 0];
                    } else if(roleBottom === pointB[1] || (roleBottom < pointA[1] && roleBottom > pointB[1])){ // 错误进入了平台内部
    ...
    getMapX(mapMoveLimitRage: number, mapData: Array<Array<number>>, mapPosition: [number, number], positionX:number, screenWidth: number, velocity:[number, number], width: number){
        if(velocity[0] > 0){ // 因为velocity[0]的方向与mapData方向协同，这里需要另行考虑。
            if(positionX < mapMoveLimitRage){
                return positionX;
            } else if(positionX > (mapData[mapData.length - 1][0]) - screenWidth + mapMoveLimitRage){
                return mapData[mapData.length - 1][0] - screenWidth + positionX
            } else {
                return  mapPosition[0] - screenWidth/2 + mapMoveLimitRage
            }
        } else if(velocity[0] < 0){
            if(positionX > mapData[mapData.length - 1][0] - mapMoveLimitRage){
                return  mapData[mapData.length - 1][0] - screenWidth + positionX;
    ...
    create(ctx: any,initialData:roleInitialData){
        // 初始数据及状态
        const {positionX, positionY, horizon, mapData, width, height, velocity, tempHorizon} = initialData;
        this.ctx = ctx;
